...
(defclass OpAnalyzer []
  (defn __init__ [self ^str test-bench-dir ^str pdk-path]
  ...
  (defn evaluate-performance [self]
    (setx self.performance (| (characterize-performance self.tb-session)
                              (characterize-operatingpoint self.dc-session))))
...
(defn characterize-operatingpoint ^dict [^SpectreInteractive session]
  (-> session (.run-all) (.values) (first) (.to-dict "records") (first)))

(defn characterize-performance ^dict [^SpectreInteractive session
